"""
🔥 CHAOS TESTING FOR SERVER.PY (Structlog Version)
===================================================

This is identical to chaos_test.py but targets server.py on port 8001.
Allows side-by-side comparison of custom logger vs structlog output!

Usage:
    Terminal 1: python server.py
    Terminal 2: python chaos_test_server.py
"""
...
class ChaosRequester:
    """Chaos testing client - identical to chaos_test.py"""
    ...
    def bad_malformed_request(self) -> Dict:
        ...
        try:
            response = self.client.get(f"{self.base_url}{endpoint}", params={param_key: param_value})
    ...
    def bad_rapid_fire_requests(self) -> Dict:
        ...
        return {
            ...
            "success": all(r in [200, 403, 503] for r in results),
            "rapid_fire": True,
            "results": results
        }
...
def print_stats():
    ...
    print(f"\n{CYAN}{'=' * 80}{RESET}")
    print(f"{BOLD}{GREEN}📊 CHAOS TEST RESULTS (SERVER.PY - STRUCTLOG){RESET}")
    ...
    print(f"\n{CYAN}{'=' * 80}{RESET}")
    print(f"{GREEN}✅ Chaos test complete! Check server.py logs for structlog output.{RESET}")
    print(f"{YELLOW}💡 Compare with starter_code.py logs to see the difference!{RESET}")
    print(f"{CYAN}{'=' * 80}{RESET}\n")
...
def main():
    """Run the chaos test"""
    print(f"\n{CYAN}{'=' * 80}{RESET}")
    print(f"{BOLD}{MAGENTA}🔥 CHAOS TESTING - SERVER.PY (STRUCTLOG){RESET}")
    print(f"{CYAN}{'=' * 80}{RESET}\n")
    print(f"{YELLOW}⚡ Testing server.py with structlog on port 8001...{RESET}\n")
    ...
    try:
        ...
        print(f"{YELLOW}   Start the server first: python server.py{RESET}\n")
        return
